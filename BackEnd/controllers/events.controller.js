...
export const updateEvent = async (req, res) => {
  try {
    ...
    if (req.body.targetItems) {
      try {
        targetItems = JSON.parse(req.body.targetItems);
        if (!Array.isArray(targetItems)) {
          return res.status(400).json({ message: "targetItems must be an array" });
        }
      } catch (error) {
        return res.status(400).json({ message: "Invalid targetItems format" });
      }
    }
    ...
    const updates = {
      ...
      shortDescription: req.body.shortDescription || event.shortDescription,
      longDescription: req.body.longDescription || event.longDescription,
      ...
      targetAmount: req.body.targetAmount || event.targetAmount,
      currentAmount: req.body.currentAmount || event.currentAmount,
      targetVolunteers: req.body.targetVolunteers || event.targetVolunteers,
      currentVolunteers: req.body.currentVolunteers || event.currentVolunteers,
      volunteerHours: req.body.volunteerHours || event.volunteerHours,
      targetItems: targetItems || event.targetItems,
      mainImage: mainImageUrl,
      images: imagesUrls,
    };
